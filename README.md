...
### 环境变量为什么每次部署都会消失，以及两类变量的区别

Cloudflare 项目里其实有两套完全不同的"环境变量"：

- **构建期变量**（比如 `VITE_CLOUDINARY_CLOUD_NAME`）：配置在 Settings -> Build -> Environment variables，只在 `npm run build` 执行时被 Vite 读取，写进打包产物里。这一套不受 `wrangler deploy` 影响，正常情况下不会消失。
- **运行期变量**（比如 `ADMIN_USER`/`ADMIN_PASSWORD`）：配置在 Settings -> Variables and Secrets，是 Worker 运行时通过 `env` 对象读取的。**每次 `wrangler deploy` 都会把 `wrangler.admin.jsonc` 当作唯一真相**：只要这个文件里没有声明某个变量，哪怕你在 Dashboard 里手动加过，也会在下一次部署时被直接清空——这是 Cloudflare 官方文档里写明的默认行为，不是 bug。

解法是在 `wrangler.admin.jsonc`（和 `wrangler.public.jsonc`）里加一行 `"keep_vars": true`，告诉 wrangler 部署时不要动 Dashboard 里已经设置的变量。这个仓库里已经加上了；如果你是照着更早版本的压缩包配置的，记得同步这一行，然后重新去 Dashboard 补一次 `ADMIN_USER`/`ADMIN_PASSWORD`（因为已经被清空的那次不会自动恢复）。
...
当前 Admin 已经改成 **结构化表单 + 双模保存**：

- **本地模式**：直接通过 Vite 提供的本地内容接口读写 `src/data/*.json`
- **远程模式**：通过 GitHub Contents API 创建 commit
- **保存前校验**：会校验双语字段、数组结构和 Cloudinary 资源对象，避免把错误内容写进 JSON
...
Admin 使用 Cloudinary **Media Library Widget**。点击“选择图片”后，用户在 Cloudinary 的登录窗口中认证、浏览或上传资源，选中的资源会直接写回后台表单中的 `{ publicId, version, format }` 字段。这里不使用 unsigned upload preset，也不会在前端暴露 API secret。
...
```
server/
└── localContentEditor.js     # 本地模式的文件读写接口

src/
├── content/
│   └── contentFiles.js       # 内容文件白名单与标签
...
│   ├── useCloudinary.js      # 选择 Cloudinary 资源
│   ├── useContentStore.js    # 本地/远程双模内容保存适配
│   └── useContentValidation.js # JSON 结构校验
├── views/
│   ├── Home.vue              # 作品集、能力矩阵、时间轴和案例
│   └── Admin.vue             # 独立构建的结构化内容后台
└── router/                   # 公开站点仅有 /
```

## 双语内容说明

4 个内容 JSON 里凡是需要展示的文本字段都写成 `{ "zh": "...", "en": "..." }` 的形式，
